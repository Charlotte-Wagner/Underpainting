...
Three changes are worth flagging here because they alter the method rather than the wording:
  ...
  - v1 said darks first, then midtones, then brightest last. The value study image contains
    true black and true white from stage 1, so the text and the image disagreed about what
    step 1 is. Resolved toward the image: both endpoints get placed first as anchors, with
    the brightest accents still saved for last, since a lightest mass and a final highlight
...
Structured in build order on purpose: values, then color masses, then soft focus, then full
detail, because that's the same order imaging.build_stages computes its four outputs in.
...
RUBRIC = """\
Before starting: the canvas and the drawing
...
before any real judgment is possible.
...
pure black and pure white from the very first stage. Everything else is judged between those
two anchors, so placing both before any midtone is what keeps the middle of the range honest.
...
Value carries most of the description of form, which is why it comes first, but color is not
only decoration: temperature does real work in pushing things back and pulling them forward,
and that becomes a live concern in the next stage. The reason for the order is that value is
the part that still reads when everything else is stripped away, not that color is optional.
...
Each stage exists to lock down a reference the next stage is judged against: the value
anchors before the midtones, the value structure before color, the block-in before detail.
...
OUTPUT_INSTRUCTIONS = (
    ...
    "an introduction, a title, or any section before the first label; the canvas toning and "
    "the initial drawing belong inside the first step, not in front of it. Do not add a "
    "closing summary after the last step.\n\n"
